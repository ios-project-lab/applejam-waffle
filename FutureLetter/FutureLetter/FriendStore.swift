...
class FriendStore : ObservableObject {
    ...
    func loadFriendsFromServer() {
        ...
        urlComponents.queryItems = [
            URLQueryItem(name: "usersId", value: String(userId))
        ]
        ...
        URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            ...
            do {
                ...
                let decodedFriends = try decoder.decode([FriendItem].self, from: data)
                
                // 4. 메인 스레드에서 @Published 변수 업데이트 (뷰 자동 갱신)
                DispatchQueue.main.async {
                    self?.friends = decodedFriends
                    print("친구 데이터 로드 및 업데이트 완료: \(decodedFriends.count)개")
                }
